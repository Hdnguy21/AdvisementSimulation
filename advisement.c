...
int   numberFreeHallSeats = 0;	// Number of seats that is free
...
void main() {
   printf("ACO350: Advisement - Ha Uyen Nguyen\n\n");
...
void *professor(void *param) {
   ...
   while (TRUE) {
      sem_wait(&studentReady); // Wait for a student to be ready
      sem_wait(&accessHallSeats); // Increase number of free seats
      numberFreeHallSeats++;
      printf("Free Seats: %d\n", numberFreeHallSeats);
      sem_post(&accessHallSeats);
      sem_post(&professorReady); // Signal professor is ready
...
void *student(void *param) {
   ...
   while (TRUE) {
   sem_wait(&accessHallSeats); // Check if seats are available
   if (numberFreeHallSeats > 0) {
      numberFreeHallSeats--; // Take a seat
      printf("Free Seats: %d\n", numberFreeHallSeats);
      sem_post(&accessHallSeats);
      sem_post(&studentReady); // Notify professor and wait
